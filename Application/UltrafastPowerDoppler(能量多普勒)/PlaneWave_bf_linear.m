% 此脚本为超快能量多普勒第二步（第一步为采集数据，第三步为能量多普勒RunUPD）
% 脚本功能：
% 适用于线阵探头
...
data_filepath = 'D:\software_matlab\exampledata\20250826NLS\20250826153835\';   %用于解析的ADC数据文件路径
adc_folder = data_filepath;
bfdata_filepath = 'D:\software_matlab\exampledata\20250826NLS\20250826153835\bfiq';   %用于保存波束合成结果的路径
...
is_show = 1; % 波束合成的时候是否显示图像 1为显示 0为不显示
...
Nx = 256;               % 波束合成线束数量x轴方向
x_start = -0.01905;     % 波束合成区域左端x坐标(m)
x_end = 0.01905;        % 波束合成区域右端x坐标(m)

Nz = 512;               % 波束合成线束数量z轴方向
z_start = 0.002;        % 波束合成区域上端z坐标(m)
z_end = 0.04;           % 波束合成区域下端z坐标(m)
...
for i = 1:numel(steer)
    if steer(i) >= 0
        tx_delay(:,i) = ((x_grid - min(AcqConfig.Probe.element_pos.x))*sin(steer(i)) + z_grid*cos(steer(i)))/sos*fs;
    else
        tx_delay(:,i) = ((max( AcqConfig.Probe.element_pos.x) - x_grid)*sin(-steer(i)) + z_grid*cos(steer(i)))/sos*fs;
    end
end

% 根据阵元指向性开窗
f_number = est_fNumber(AcqConfig.Probe.element_pitch,lambda,0.81);
...
%  sample_data = gpuArray(single(sample_data));

if is_show
    hFig = figure('Name',"planewave reconstruction",'NumberTitle','off');
    hIm = imagesc(x_axis*100,z_axis*100,zeros(Nz,Nx));
...
save(fullfile(bf_folder,"bfiq_"+pack_num_idx+".mat"),"bfiq","x_axis","z_axis")
